class Joystick {
    constructor(x, y, br, kr, bc, kc) {
        this.x = x || 0;
        this.y = y || 0; // Top-left coords
        this.br = br || 60;
        this.kr = kr || 20; // Radius (background, knob)
        this.bc = bc || "#777";
        this.kc = kc || "#FFF";
        this.kx = 0;
        ...
        this.canvas = this.newCanvas(this.br * 2, this.br * 2);
        ...
        // For mouse drag
        var isdown = false;
        // Correct touch event (see bug log)
        var touch;
        var touchObj;
        ...
        this.addEventListeners(self.canvas, ["mousedown", "touchstart"], function(e) {
            ...
            if (e.touches) { // If touch (not mousedown)
                for (var i = 0; i < e.touches.length; i++) {
                    if (e.touches[i].identifier == e.targetTouches[0].identifier) {
                        touch = i;
                    }
                }
                
                self.kx = e.touches[touch].clientX - self.x - self.w / 2;
                self.ky = e.touches[touch].clientY - self.y - self.h / 2;
            } else {
    ...
    update() {
        ...
        c.fillStyle = this.rgba(this.bc); // FIX (not always rgba)
        c.fill();
        
        c.beginPath();
        c.arc(this.kx, this.ky, this.kr, 0, 2 * Math.PI);
        c.fillStyle = this.rgba(this.kc); // FIX
        c.fill();
    }
